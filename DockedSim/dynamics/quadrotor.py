...
from utils.transform import deg2rad, euler2quat, quat2rot, rot2euler  # quat2euler,
...
class Drone():
    ...
    def __init__(self):
        ...
        self.dock_port_inB_pos = np.array([0.1, 0, 0])
    ...
    def step(self, u, modular_flag = False):
        """
        RK45 integrator for one step/dt
        :param modular_flag: use modular control when docked
        ...
        if modular_flag is True:
            self.u = self.thrust_to_force(u)
    ...
    def attitude_limit(self):
        ...
        euler = rot2euler(quat2rot(self.state[6:10]))
        r = euler[0]
        p = euler[1]
        y = euler[2]
    ...
    def thrust_to_force(self, thrust):
        general_force = self.rotor2control @ thrust
